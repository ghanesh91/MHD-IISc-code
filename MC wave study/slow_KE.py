...
def kzkrmax2(n,arB,azB,ARmin,AZmin,constr,constz,t):
       ...
       I=32*np.pi**4*np.trapz(kr_datab*np.trapz(np.real(uB1)**2,kz_datab),kr_datab)
...
if __name__=='__main__':
    ...
    for n in range(restart_num,41):
       ...
       for i in range(np.size(t)):
          if rank==0:print("computing integrals at t/tA=",t[i]/t_Omega,'num=',i+n*11,'kz_max',azB[n])
          [uhat_theta,duhat_theta_dt,uB1,uD1]=u_hat_theta(KZB,KRB,KB,l1b,l2b,l3b,l4b,uhat0b,t[i])
          
          integral_qty[0].append(t[i])
          integral_qty[1].append(32*np.pi**4*np.trapz(kr_datab*np.trapz(np.real(uB1)**2,kz_datab),kr_datab))
          integral_qty[2].append(32*np.pi**4*np.trapz(kr_datab*np.trapz(np.abs(KRB*np.real(uB1))**2,kz_datab),kr_datab))
          integral_qty[3].append(32*np.pi**4*np.trapz(kr_datab*np.trapz(np.abs(KZB*np.real(uB1))**2,kz_datab),kr_datab))
          integral_qty[4].append(32*np.pi**4*np.trapz(kr_datab*np.trapz(np.abs(KB*np.real(uB1))**2,kz_datab),kr_datab))
          ...
          if i==(np.size(t)-1):#i+n*11 == 65:                
             ...
             if rank==0:
                ...
                #os.remove('inertial_Alfven_Els1e-3_Rm10_Ro1e-2_Pm1e-10_2e4x2e4.mat')
                io.savemat('inertial_Alfven_s_Els1e-3_Rm10_Ro1e-2_Pm1e-10_2e4x2e4.mat', mdict={quantity[0][i]:quantity[1][i][:] for i in range(np.size(quantity[0]))},appendmat=False) 
