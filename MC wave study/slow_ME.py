...
def distribute(kz,procs,chunks):
   """Creating "procs" number of empty chunks for storing the ks and kz values that is to be divided across procs. """
...
def b_hat_theta(KZ,KR,K,l1,l2,l3,l4,uhat0,t):
   ...
   return jhat_r,jhat_z,bhat_theta,djhat_r_dt,djhat_z_dt,dbhat_theta_dt,bB
"""************************************************************************************************************************************"""
...
def kzkrmax2(n,arB,azB,ARmin,AZmin,constr,constz,t):
       ...
       [jhat_r,jhat_z,bhat_theta,djhat_r_dt,djhat_z_dt,dbhat_theta_dt,bB1]=b_hat_theta(KZB,KRB,KB,l1b,l2b,l3b,l4b,uhat0b,t)
       I=32*np.pi**4*np.trapz(kr_datab*np.trapz(np.imag(bB1)**2,kz_datab),kr_datab)
...
if __name__=='__main__':
    ...
    if restart_num == 0:
       ...
       if rank==0:
          print("initial run")
          f=h5py.File('inertial_Alfven_s_Els1e-3_Rm10_Ro1e-2_Pm1e-10_2e4x2e4.h5','w')
    ...
    if restart_num > 0:
       ...
       for i in range(np.size(f['t'])):
          azB[i]=f['kz'][0,i]
          arB[i]=f['kr'][0,i]  
       F=io.loadmat('inertial_Alfven_s_Els1e-3_Rm10_Ro1e-2_Pm1e-10_2e4x2e4.mat')
       quantity=[ [  ]  ,  [[  ] for _ in range(np.size(variables))]]
    ...
    for n in range(restart_num,41):
       ...
       if n < 25:
          criteria=1e-10      
          ARmin=1e-4*(Lr/(2*np.pi));
          AZmin=1e-4*(Lz/(2*np.pi)); 
       else:
          criteria=1e-10
          ARmin=1e-4*(Lr/(2*np.pi));
          AZmin=1e-4*(Lz/(2*np.pi)); 
       ...
       for i in range(np.size(t)):
          ...
          [jhat_r,jhat_z,bhat_theta,djhat_r_dt,djhat_z_dt,dbhat_theta_dt,bB]=b_hat_theta(KZB,KRB,KB,l1b,l2b,l3b,l4b,uhat0b,t[i])
          
          integral_qty[0].append(t[i])
          integral_qty[1].append(32*np.pi**4*np.trapz(kr_datab*np.trapz(np.imag(bB)**2,kz_datab),kr_datab))
          integral_qty[2].append(32*np.pi**4*np.trapz(kr_datab*np.trapz(np.abs(KRB*np.imag(bB))**2,kz_datab),kr_datab))
          integral_qty[3].append(32*np.pi**4*np.trapz(kr_datab*np.trapz(np.abs(KZB*np.imag(bB))**2,kz_datab),kr_datab))
          integral_qty[4].append(32*np.pi**4*np.trapz(kr_datab*np.trapz(np.abs(KB*np.imag(bB))**2,kz_datab),kr_datab))
          ...
          if i==(np.size(t)-1):#i+n*11 == 65:                
             ...
             if rank==0:
                ...
                io.savemat('inertial_Alfven_s_Els1e-3_Rm10_Ro1e-2_Pm1e-10_2e4x2e4_ME.mat', mdict={quantity[0][i]:quantity[1][i][:] for i in range(np.size(quantity[0]))},appendmat=False) 
