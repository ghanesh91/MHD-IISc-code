...
Els=0.001;Rm=10;d=0.25000;Uo=0.10718*1e6;Ro=1e-2;Pm=1e-10;
L=np.sqrt((Els*Rm)/Ro)
...
def kzkrmax2(n,arB,azB,ARmin,AZmin,constr,constz,t):
       ...
       [uhat_theta,duhat_theta_dt,uB1,uD1]=u_hat_theta(KZB,KRB,KB,l1b,l2b,l3b,l4b,uhat0b,t)
       I=32*np.pi**4*np.trapz(kr_datab*np.trapz(np.real(uD1)**2,kz_datab),kr_datab)
...
if __name__=='__main__':
    ...
    if restart_num == 0:
       ...
       if rank==0:
          print("initial run")
       #   f=h5py.File('inertial_Alfven_f_Els0.001_Rm10_Ro1e-2_Pm1e-10_2e4x2e4.h5','w')
    ...
    if restart_num > 0:
       f=io.loadmat('wavenumber_Els0.001.mat')
       for i in range(np.size(f['t'])):
          azB[i]=f['kz'][0,i]
          arB[i]=f['kr'][0,i]  
       F=io.loadmat('inertial_Alfven_f_Els0.001_Rm10_Ro1e-2_Pm1e-10_2e4x2e4.mat')
       quantity=[ [  ]  ,  [[  ] for _ in range(np.size(variables))]]
    ...
    for n in range(restart_num,41):
       ...
       if n < 20:
          ...
          criteria=1e-10      
          pdiff_criteria=1e-2
          ARmin=1e-6*(Lr/(2*np.pi));
          AZmin=1e-6*(Lz/(2*np.pi)); 
       else:
          Npt_kr=2e4
          Npt_kz=2e4
          criteria=1e-10
       ...
       for i in range(np.size(t)):
          if rank==0:print("computing integrals at t/tA=",t[i]/tA,'num=',i+n*11,'kz_max',azB[n])
          [uhat_theta,duhat_theta_dt,uB1,uD1]=u_hat_theta(KZB,KRB,KB,l1b,l2b,l3b,l4b,uhat0b,t[i])
          
          integral_qty[0].append(t[i])
          integral_qty[1].append(32*np.pi**4*np.trapz(kr_datab*np.trapz(np.real(uD1)**2,kz_datab),kr_datab))
          integral_qty[2].append(32*np.pi**4*np.trapz(kr_datab*np.trapz(np.abs(KRB*np.real(uD1))**2,kz_datab),kr_datab))
          integral_qty[3].append(32*np.pi**4*np.trapz(kr_datab*np.trapz(np.abs(KZB*np.real(uD1))**2,kz_datab),kr_datab))
          integral_qty[4].append(32*np.pi**4*np.trapz(kr_datab*np.trapz(np.abs(KB*np.real(uD1))**2,kz_datab),kr_datab))
          ...
          if  i==(np.size(t)-1):
             ...
             #if rank==0:
             #   f['kz_maxB'][0,n+1]=azB[n+1]
             #   f['kr_maxB'][0,n+1]=arB[n+1]
             #   f['time_tot'][0,n+1]=t_tA[n+1]*tA
             #   print('kr_maxB',arB)
             #   print('kz_maxB',azB)
          ...
          if i==(np.size(t)-1):                
             ...
             if rank==0:
                ...
                #os.remove('inertial_Alfven_Els0.001_Rm10_Ro1e-2_Pm1e-10_5e3x5e3.mat')
                io.savemat('inertial_Alfven_f_Els0.001_Rm10_Ro1e-2_Pm1e-10_2e4x2e4.mat', mdict={quantity[0][i]:quantity[1][i][:] for i in range(np.size(quantity[0]))},appendmat=False)
                io.savemat('wavenumber_Els0.001.mat', mdict={'kr':arB,'kz':azB,'t':t_tA},appendmat=False) 
